...
class combine_model(nn.Module):
    def __init__(self, ENT_NUM, triples):
        ...
        # self.entity_emb = entity_emb.to(device)
        # self.cross_graph_model = GCN(n_units[0], n_units[1], n_units[2], dropout=0).to(device)
        # self.input_idx = torch.LongTensor(np.arange(ENT_NUM)).to(device)
        # input_dim = int(n_units[0])
        # entity_emb = nn.Embedding(ENT_NUM, input_dim)
        # nn.init.normal_(entity_emb.weight, std=1.0 / math.sqrt(ENT_NUM))
        # entity_emb.requires_grad = True
        ...
        self.bert = Basic_Bert_Unit_model(MODEL_INPUT_DIM, MODEL_OUTPUT_DIM).to(device)
    ...
    def forward(self, batch_word_list, attention_mask, eids):
        dcb_emb = self.bert(batch_word_list, attention_mask)
        gph_emb = self.cross_graph_model(self.entity_emb.weight, self.adj)
        joint_emb = torch.cat([
            F.normalize(dcb_emb),
...
if __name__ == '__main__':
    with open(DATA_PATH + "feature/ja_basic.npy", 'rb') as f:
        d_feature = np.load(f)
    print(type(torch.Tensor(d_feature).to(device)))
