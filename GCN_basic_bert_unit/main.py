...
#     return batch_emb
def entlist2emb(Model, entids, entid2data, cuda_num):
    # --- 消融实验：去除 att & desc (文本信息) ---
    # 我们可以直接返回一个全 0 向量，或者只保留 Model 中的结构部分得分
    # 获取模型输出的维度（通常是 MODEL_OUTPUT_DIM）

    # 逻辑修改：
    batch_token_ids = torch.LongTensor(len(entids), 128).fill_(0).to(device)  # 假设长度128
    batch_mask_ids = torch.FloatTensor(len(entids), 128).fill_(0).to(device)

    # 如果 Model 内部支持只跑 GCN/结构分支，则调用对应分支
    # 如果 Model 强依赖 BERT 输入，则传入 0 值：
    batch_emb = Model(batch_token_ids, batch_mask_ids, entids)
...
def main():
    ...
    if RANDOM_DIVIDE_ILL:
        ...
        # get train/test ILLs from ill_ent_ids
        print("Get train/test ILLs from ill_ent_ids ({}% for training)".format(int(TRAIN_ILL_RATE * 100)))
        print("train ILL num: {}, test ILL num: {}".format(len(train_ill), len(test_ill)))
