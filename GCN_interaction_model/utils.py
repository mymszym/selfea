...
def batch_topk(mat,bs=128,topn = 50,largest = False,cuda_num = 0):
    ...
    res_index = []
    axis_0 = mat.shape[0]
    # Ensure topn does not exceed the actual size
    actual_topn = min(topn, mat.shape[1])
    for i in range(0,axis_0,bs):
        temp_div_mat = mat[i:min(i+bs,axis_0)].to(device)
        score_mat,index_mat =temp_div_mat.topk(actual_topn,largest=largest)
...
def test_topk_res(index_mat):
    ...
    # Return hit@1 and hit@max(50, ent2_num)
    hit50 = topk_list[50 - 1] if ent2_num >= 50 else topk_list[-1]
    return topk_list[1 - 1], hit50
