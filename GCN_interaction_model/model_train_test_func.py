...
def one_step_train(Model, Optimizer, Criterion, Train_gene, f_emb, cuda_num):
    ...
    for pos_f_ids, neg_f_ids in Train_gene:
        Optimizer.zero_grad()
        pos_feature = f_emb[torch.LongTensor(pos_f_ids)].to(device)
        neg_feature = f_emb[torch.LongTensor(neg_f_ids)].to(device)
        ...
        label_y = torch.ones(p_score.shape).to(device)  # if y == 1 mean: p_score should ranked higher.
...
def test(Model, test_candidate, test_ill, entpair2f_idx, f_emb, batch_size, cuda_num, test_topk):
    ...
    MRR /= all_test_num
    print("MRR:", MRR)
    return topk_list[1 - 1], topk_list[10 - 1], MRR
...
def train(Model, Optimizer, Criterion, Train_gene, f_emb_list, test_candidate, test_ill,
    ...
    maxHit1, maxHit10, maxMRR, epc = 0, 0, 0, 0
    for epoch in range(epoch_num):
        ...
        if (epoch + 1) % eval_num == 0 and epoch != 0:
            start_time = time.time()
            hit1, hit10, mrr = test(Model, test_candidate, test_ill, entpair2f_idx, feature_emb, 2048, cuda_num, test_topk)
            if maxHit1 < hit1:
                maxHit1, maxHit10, maxMRR, epc = hit1, hit10, mrr, epoch
            print("test using time {:.3f}".format(time.time() - start_time))
    saveResult(maxHit1, maxHit10, maxMRR, epc)


def saveResult(hit1, hit10, mrr, epc):
