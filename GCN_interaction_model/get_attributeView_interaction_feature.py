...
def attributeView_interaction_F_gene(ent_pairs, value_emb_list, ent2valueids, value_pad_id,
    ...
    value_emb = torch.FloatTensor(value_emb_list).to(device)
    ...
    for start_pos in range(0, len(ent_pairs), batch_size):
        ...
        e1_values = torch.LongTensor(e1_values).to(device)   # [B,ne1]
        e2_values = torch.LongTensor(e2_values).to(device)   # [B,ne2]
