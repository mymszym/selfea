...
def test_read_emb(ent_emb, train_ill, test_ill, bs=128, candidate_topk=50):
    ...
    print(f"train_ids_1: {len(train_ids_1)}, train_ids_2: {len(train_ids_2)}")
    print(f"test_ids_1: {len(test_ids_1)}, test_ids_2: {len(test_ids_2)}")
...
def neighborView_interaction_F_gene(ent_pairs, ent_emb_list, neigh_dict, ent_pad_id,
    ...
    e_emb = torch.FloatTensor(ent_emb_list).to(device)
    e_emb = F.normalize(e_emb, p=2, dim=-1)
    sigmas = kernel_sigmas(kernel_num).to(device)
    mus = kernel_mus(kernel_num).to(device)
    # print("sigmas:",sigmas)
    # print("mus:",mus)
    ...
    for start_pos in range(0, len(ent_pairs), batch_size):
        ...
        e1_masks = torch.FloatTensor(e1_masks.tolist()).to(device) .unsqueeze(-1)  # [B,ne1,1]
        e2_masks = torch.FloatTensor(e2_masks.tolist()).to(device) .unsqueeze(-1)  # [B,ne2,1]
        e1_tails = torch.LongTensor(e1_tails).to(device)   # [B,ne1]
        e2_tails = torch.LongTensor(e2_tails).to(device)   # [B,ne2]
...
def desornameView_interaction_F_gene(ent_pairs, e_emb_list,
    ...
    start_time = time.time()
    e_emb = torch.FloatTensor(e_emb_list).to(device)
    all_ent_pairs = []
    all_features = []
    ...
    for start_pos in range(0, len(ent_pairs), batch_size):
        ...
        e2s = [e2 for e1, e2 in batch_ent_pairs]
        e1s = torch.LongTensor(e1s).to(device)
        e2s = torch.LongTensor(e2s).to(device)
...
def main():
    ...
    # Load train_ill and test_ill from ill_ent_ids
    ills = []
